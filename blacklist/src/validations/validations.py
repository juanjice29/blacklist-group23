...
def obtener_password(passw,correo):
    return passw.strip() + base64.b64encode(correo.strip().split('@')[0].encode()).decode('utf-8')
...
def obtener_fecha_actual():
    return datetime.utcnow().isoformat() if not os.environ.get("ENV") == 'test' else datetime.utcnow()